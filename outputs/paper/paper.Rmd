...
bio_results <- 
  read_csv(here::here("outputs/data/bio_results.csv")) # All COVID bioRxiv papers (3,031)
...
The result of this process is a dataset indicating the presence of open data or open code markers in each pre-print (with a binary variable for each marker, and the relevant open data or open code statements when applicable). Our final analysis dataset was formed by joining these two datasets together, typically using the DOI or the unique filename, to form a dataset including all original, qualitative metadata information for each pre-print alongside its open data and open code status and markers. 
...
all %>% 
  ...
  labs(title = "Pre-prints posted per month",
       subtitle = "Total of 3,379 relevant pre-prints on arXiv, 3,031 on bioRxiv, 9,929 on medRxiv",
       x = "Month",
...
Broadly, we are unable to find markers of either open data or open code for around 81 per cent of the 1,200 sampled pre-prints. The remaining 19 per cent of pre-prints was comprised of 4 per cent of the sample containing markers for open code, 8 per cent of the sample with markers for open data, and 7 per cent of the sample with markers for both open data and open code (Table \@ref(tab:summarycounts-medRxiv)).
...
bio_open_data_results %>% 
  ...
  mutate(is_open_data = ifelse(is_open_data == 0, "No", "Yes"),
         is_open_code = ifelse(is_open_code == 0, "No", "Yes")) %>% 
  kable(col.names = c("Open data markers", "Open code markers", "Count", "Proportion of total"),
        caption = "Counts and proportions of open data and code markers in bioRxiv sample",
        booktabs = TRUE,
        digits = 2)
```
