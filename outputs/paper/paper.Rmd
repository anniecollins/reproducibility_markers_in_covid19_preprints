...
med_results <- 
  read_csv(here::here("outputs/data/med_results.csv")) # All COVID medRxiv papers (9,929)

arxiv_open_data_results <- 
  read_csv(here::here("outputs/data/arxiv_open_data_results.csv"))
all_arxiv_results <- 
  read_csv(here::here("outputs/data/arxiv_results.csv"))
...
```{r monthly-papers-total, fig.cap = "Number of pre-prints related to COVID-19 posted to arXiv and medRxiv", out.width="90%", fig.align = "center"}
just_arxiv <- 
  all_arxiv_results  %>% 
  select(submitted) %>% 
  mutate(type = "arXiv") %>% 
...
both <- rbind(just_arxiv, just_medRxiv)
...
both %>% 
  ...
  labs(title = "Pre-prints posted per month",
       subtitle = "Total of 3,379 on arXiv and 9,929 on medRxiv",
       x = "Month",
