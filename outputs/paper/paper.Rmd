...
socarxiv_results <- 
  read_csv(here::here("outputs/data/socarxiv_results.csv")) # All COVID medRxiv papers (9,929)  
...
all %>% 
  ...
  labs(title = "COVID-19-related pre-prints posted per month",
       subtitle = "Total of 3,812 relevant pre-prints on arXiv, 3,714 on bioRxiv, 12,060 on medRxiv, 444 on socArXiv",
       x = "Month",
...
published_open_status <- rbind(med_published_open_status, bio_published_open_status)
...
published_open_status %>%
  ...
  select(Published, Both, Neither, `Open code`, `Open data`, `Proportion with either/both`) %>%
